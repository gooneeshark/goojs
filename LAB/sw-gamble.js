// Service worker that intercepts /api/* calls to simulate a gambling backend
...
async function handleApi(req) {
  ...
  // simple auth check: header x-api-key required
  ...
  try {
    ...
    if (path === 'spin' && req.method === 'POST') {
      ...
      // simulated rate limit: occasional 429
      ...
      // spin logic: 3 reels, symbols 0..5
      const reels = [randSym(), randSym(), randSym()];
      let payout = 0;
      if (reels[0] === reels[1] && reels[1] === reels[2]) {
        payout = bet * (10 + reels[0]); // big win
      ...
      // variable delay to encourage handling slow responses
